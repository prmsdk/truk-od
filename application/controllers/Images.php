...
class Images extends CI_Controller
{
    ...
    public function index()
	{
        ...
        $data['navParent'] = 'images';
        $data['data'] = $this->crud->getAll("images")->result();
    ...
    {
        $data = $this->crud->getAll("images")->result();

        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();

        $sheet->mergeCells("A1:O1");
        $sheet->setCellValue('A1', 'Laporan Data Images All');
        ...
        $filename = 'laporan-imagesAll-' . date('Y-m-d') . date('his');
